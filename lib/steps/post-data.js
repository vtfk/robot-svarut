...
const postAndWait = async (data) => {
  ...
  if (innerResult && innerResult.data.customStatus && innerResult.data.customStatus === 'Failed') {
    ...
    if (innerResult.data.output.map(sentAlready).includes(false)) {
      await logger('warn', ['post-data-and-await', data._id, 'customStatus', innerResult.data.customStatus])
      throw innerResult.data
    } else {
      await logger('warn', ['post-data-and-await', data._id, 'customStatus', innerResult.data.customStatus, 'however, all is sent OK'])
    }
  }
...
module.exports = data => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    logConfig({
      teams: {
        level: 'warn',
        url: config.EMANUEL_TEAMS_WEBHOOK
      }
    })
    ...
    try {
      ...
      await logger('error', ['post-data', data._id, error])
      data.retry = true
